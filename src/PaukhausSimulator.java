		...
		try (Scanner scan = new Scanner(System.in)) {
			...
			while (a != 0) {
				...
				while (eingabe1 == 2) {
					...
					if (eingabe3 == 1 && t1.getBezahlt() == false) {
						...
						if (DateHelper.calculateTimeDifferenceInMinutes(t1.getDatEingang(), t1.getDatKasse()) >= 60) {
							parkgebuehr = parkgebuehr + Math.round(
									(DateHelper.calculateTimeDifferenceInMinutes(t1.getDatEingang(), t1.getDatKasse())
											/ 30)
											* 0.50);
						}
					...
					// Ticket 2
						if (eingabe3 == 2 && t2.getBezahlt() == false) {
							t2.setDatKasse(DateHelper.getCurrentTimeAsString());
							parkgebuehr = 2;
							if (DateHelper.calculateTimeDifferenceInMinutes(t2.getDatEingang(), t2.getDatKasse()) >= 60) {
								parkgebuehr = parkgebuehr + Math.round(
										(DateHelper.calculateTimeDifferenceInMinutes(t2.getDatEingang(), t2.getDatKasse())
												/ 30)
												* 0.50);
							}
							if (t2.getBezahlt() == false) {
								System.out.println("Ihre Parkgebuehr betraegt :" + parkgebuehr);
								System.out.println("(nur in der simulation) Wieviel Euro stecken sie in den Automaten?");
								gezahlt = scan.nextFloat();
								while (gezahlt < parkgebuehr) {
						...
						if (eingabe3 == 1 && t2.getBezahlt() == true) {
							System.out.println("Ihr gewähltes Ticket ist bereits bezahlt");
						}
						if (gezahlt - parkgebuehr >= 0) {
							t2.setBezahlt(true);
